...
def binance_futures_klines(pair: str, interval: str = "4h", limit: int = 120) -> pd.DataFrame:
...
def binance_spot_klines(pair: str, interval: str = "4h", limit: int = 80) -> Optional[pd.DataFrame]:
    try:
...
def coinglass_df(path: str, params: Dict[str, Any]) -> Optional[pd.DataFrame]:
    if not CG_KEY:
        return None
    try:
        data = get_json(
            COINGLASS_BASE + path,
            params,
            {"CG-API-KEY": CG_KEY, "accept": "application/json"},
        )
...
def analyze(symbol: str, interval: str = "4h") -> Result:
    ...
    df = binance_futures_klines(pair, interval, 120)
    ...
    prev_low = df["low"].tail(60).head(30).min()
    ...
    vol_now = df["quote_volume"].tail(3).mean()
    vol_base = df["quote_volume"].tail(60).head(45).mean()
    vol_ratio = float(vol_now / vol_base) if vol_base else 1.0
    volume = min(max((vol_ratio - 1.0) * 10, 0), 20)

    spot_volume = 5
    spot_vol_ratio = 1.0
    sdf = binance_spot_klines(pair, interval, 80)
    if sdf is not None and len(sdf) >= 50:
        s_now = sdf["quote_volume"].tail(3).mean()
        s_base = sdf["quote_volume"].tail(50).head(35).mean()
        spot_vol_ratio = float(s_now / s_base) if s_base else 1.0
        spot_volume = min(max((spot_vol_ratio - 1.0) * 10, 0), 15)
    ...
    if oi_df is not None and "close" in oi_df.columns and len(oi_df) >= 6:
        oi_change = pct(safe_float(oi_df["close"].iloc[-1]), safe_float(oi_df["close"].iloc[-6]))
        oi_score = min(max(oi_change * 1.2, 0), 20)

    chg_24 = pct(df["close"].iloc[-1], df["close"].iloc[-7]) if interval == "4h" and len(df) >= 7 else 0
    risk = 10
    if chg_24 > 40:
        risk -= 5
    if vol_ratio > 8 and chg_24 > 25:
        risk -= 3
    risk = max(risk, 0)
    ...
    return Result(
        ...
        risk=round(risk, 2),
        funding_pct=round(fr, 4),
        oi_change_pct=round(oi_change, 2),
        vol_ratio=round(vol_ratio, 2),
        spot_vol_ratio=round(spot_vol_ratio, 2),
        note=" / ".join(notes) if notes else "양호",
    )
...
def format_result(r: Result) -> str:
    return (
        ...
        f"선물Vol: {r.vol_ratio}x | 현물Vol: {r.spot_vol_ratio}x\n"
...
def alert_job():
    results = scan(DEFAULT_SYMBOLS)
    hits = [r for r in results if r.score >= SCORE_ALERT]
    if hits:
